...
function validateEmployee(employee) {
    ...
    for (let key in employee) {
        console.log(employee[key], key, 'key');
      employee[key] = employee[key].toString().trim();
    }
  
    const schema = Joi.object({
      employeeId: Joi.number().min(1).max(10000).required(),
      email: Joi.string().pattern(new RegExp('^([a-zA-Z0-9_\\-\\.]+)@([a-zA-Z0-9_\\-\\.]+)\.([a-zA-Z]{2,5})$')).min(5).max(1000).required(),
      name: Joi.string()
        .min(2)
        .max(100)
        .required(),
      designation: Joi.string().valid("intern","consultant1","consultant2","associate1","associate2","manager").required(),
...
async function checkDuplicate(employeeId, email){
    var isExistEmployeeId = await model.employee.getByCriteria({ employeeId: employeeId });
