...
export class CandidateAssessmentService {
  ...
  getCriteriaType(jdId: string): Observable<IResponse> {
    ...
    return this.http.get<IResponse>(`${ASSESSMENT_FILTERED_API}`, { ...this.options, params });
  }

  storeAssessment(data: any): Observable<IResponse> {
    return this.http.post<IResponse>(CANDIDATE_ASSESSMENT_API, data, this.options);
  }

  getAssessment(data: any): Observable<IResponse> {
    return this.http.get<IResponse>(`${CANDIDATE_ASSESSMENT_API}/round/${data.roundId}/application/${data.applicationId}`, { ...this.options });
  }
}
